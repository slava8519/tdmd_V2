...
class AnchorTestReport:
    ...
    # One of the seven identifiers from acceptance_criteria.md
    ...
    def format_console_summary(self) -> str:
        """Terse console summary — one line per point + a footer verdict."""
        lines: list[str] = []
        lines.append(
            f"T3 anchor-test — {len(self.points)} point(s), hw_ratio={self.hardware.ghz_flops_ratio:.3f}"
        )
