...
- **OQ-M3-5 — Decomp2D N_min formula vs §8.3 16×5 anchor.** **RESOLVED at T3.7 (2026-04-18).** SPEC §3.2 formula `N_min = 2·(N_inner + 1)` stays; `zoning/SPEC.md §8.3` table corrected to `n_opt = 6` for 16×5. Rationale: dissertation eq. (45) re-extracted during T3.4 evaluates to 14 (not the original §8.3 "13"), so the prior anchor was a SPEC transcription error — module formula is the source of truth. `[!mayfail]` tag removed from the T3.4 anchor test.
- **OQ-M3-6 — Hilbert3D 16³ anchor = 64 vs formula = 4.** **RESOLVED at T3.7 (2026-04-18).** SPEC §8.3 row "3D Hilbert 16³ → n_opt=64" was a row-label mis-attribution: 64 is the correct value for **256³** under the §3.3 formula, not for 16³. Anchor table now reads `n_opt = 4` for 16³ with an added 256³ → n_opt = 64 row for scale validation.
