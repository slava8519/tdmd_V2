...
**Last updated:** 2026-04-19 (T7.2 — Pattern 2 SPEC integration: §2.4 OuterSdCoordinator contract, §2.5 SubdomainBoundaryDependency dep kind, §4.6 HaloSnapshot archive)
...
### 2.4. OuterSdCoordinator contract (Pattern 2, M7+)

Канонический интерфейс взят из мастер-специи §12.7a. Здесь — **поведенческий контракт**, который `InnerTdScheduler` имеет право **предполагать** при работе с outer coordinator'ом, и который любая реализация `OuterSdCoordinator` обязана **гарантировать**.
...
struct SubdomainGrid {
    ...
    std::vector<Box>         subdomain_boxes;  // size == prod(n_subdomains)
...
public:
    ...
    // Predicate: может ли граничная зона `local_zone` сейчас перейти в state Computing
    // на time_level `target_level`? Возвращает true ⟺ для каждого peer subdomain'а,
    // halo которого требуется для `(local_zone, target_level)`, в архиве coordinator'а
    // присутствует HaloSnapshot для (peer_zone, target_level - 1).
    virtual bool  can_advance_boundary_zone(
        ZoneId local_zone, TimeLevel target_level) = 0;

    // Phase B хук: после успешного `commit_completed()` граничной зоны
    // InnerTdScheduler регистрирует свой HaloSnapshot в архиве coordinator'а
    // (для последующего fetch соседним subdomain'ом через outer comm backend).
    virtual void  register_boundary_snapshot(
        ZoneId local_zone, TimeLevel level, const HaloSnapshot&) = 0;

    // Pull side: при поступлении HaloPacket'а от соседнего subdomain (через
    // `comm.drain_halo_arrived()`), runtime вызывает этот метод, чтобы достать
    // unpacked snapshot для конкретной peer zone. Возвращает std::nullopt,
    // если ещё не пришло.
    virtual std::optional<HaloSnapshot>  fetch_peer_snapshot(
        uint32_t peer_subdomain, ZoneId peer_zone, TimeLevel level) = 0;

    // Boundary-specific watchdog. Отдельный от §8 deadlock'а: те зоны, что
    // стоят на boundary stall дольше `T_stall_max`, эскалируются в
    // diagnostic dump через outer coordinator (а не через inner scheduler).
    virtual void  check_stall_boundaries(std::chrono::milliseconds T_stall_max) = 0;

    // Глобальный temporal frontier поверх всех subdomain'ов:
...
**Behavioural contract** (must hold for any `OuterSdCoordinator` implementation):

| # | Гарантия | Кому нужно |
|---|---|---|
| OC-1 | `can_advance_boundary_zone(z, t)` — **non-blocking** и **idempotent**. Может вызываться > 1 раза за iteration без побочных эффектов. | `select_ready_tasks()` опрашивает на каждом шаге frontier walk |
| OC-2 | `register_boundary_snapshot(z, t, snap)` после Phase B — **одно-в-одно** соответствие: ровно один snapshot per (zone, level) pair, повторная регистрация для уже архивированного `(z, t)` — error. | `commit_completed()` граничной зоны |
| OC-3 | Архив хранит **последние `K_max` snapshot levels** на boundary zone (см. §4.6); eviction идёт по `(level)` ascending после получения ACK от peer (`fetch_peer_snapshot` increment use-count). | пиковый bound = `O(K_max × surface_zones × atom_record)` per subdomain |
| OC-4 | `fetch_peer_snapshot` — **non-blocking**, возвращает `nullopt` при отсутствии данных (не ждёт). Polling делает inner scheduler через перевыдачу select. | wait-free inner scheduler hot path |
| OC-5 | `global_frontier_min()` / `_max()` — **monotonic non-decreasing**. Реализация консолидирует через outer `comm.barrier()` или `comm.global_sum_double` на end-of-iteration. | inner cert builder: `frontier_margin = K_max·dt - (level - global_frontier_min)·dt` |
| OC-6 | `check_stall_boundaries` — отдельный watchdog, эмиттит `OuterStallReport`; не дублирует inner deadlock detection (§8). | telemetry `scheduler.boundary_stalls_total` |

В **Pattern 1** `OuterSdCoordinator*` равен `nullptr`; `select_ready_tasks()` ветвь `is_boundary_zone(z) → outer_coordinator->can_advance_…` пропускается (см. §5.1 псевдокод). Pattern 1 всегда видит `attach_outer_coordinator(nullptr)` — это допустимый initial state, не ошибка.

В **Pattern 2** существует **ровно один** экземпляр на run, владеется `SimulationEngine` (см. §12.8 мастер-специи); inner scheduler получает borrowed pointer через `attach_outer_coordinator(coord)`.

### 2.5. SubdomainBoundaryDependency — пятый dep kind

`ZoneTask::dep_mask` (см. §2.1) — bitset over the dep taxonomy из §6.3 мастер-специи. Битовое распределение:

| Bit | Dep kind | Активен в Pattern |
|---|---|---|
| 0 | Spatial neighbours | 1, 2 |
| 1 | Temporal frontier | 1, 2 |
| 2 | Certificate validity | 1, 2 |
| 3 | Neighbor list freshness | 1, 2 |
| **4** | **SubdomainBoundary** (этот раздел) | **2 only** |
| 5–63 | reserved | — |

Bit 4 устанавливается, когда `is_boundary_zone(z) == true` (зона лежит в пределах `r_c + r_skin` от subdomain border) **и** `outer_coordinator->can_advance_boundary_zone(z, target_level) == false`. Снимается, когда coordinator вернул `true` (peer snapshot прибыл) или зона сместилась во внутренний слой (например, после migration — редко, но возможно).

**Семантика релиза:**

```
on HaloArrived(peer_subdomain, peer_zone, level):
    outer_coordinator->fetch_peer_snapshot(peer_subdomain, peer_zone, level) → snap
    archive snap into coordinator's halo snapshot ring buffer (§4.6)
    re-evaluate dep_mask for all local zones whose can_advance_boundary_zone changed
```

**Property test** (расширение fuzzer'а §12.2 для Pattern 2):

```
forall seed in fuzz_seeds:
    grid = generate_subdomain_grid(seed, P_space ∈ {2,4,8})
    coord = MockOuterSdCoordinator(grid, K_max)
    inner = CausalWavefrontScheduler(Reference)
    inner.attach_outer_coordinator(&coord)
    events = generate_mixed_events(seed, N=10000)  # включая HaloArrived
    for event in events:
        apply(inner | coord, event)
        # Pattern-2-specific invariants:
        assert  forall task ∈ ready_queue:  bit4_clear(task.dep_mask) == true
        assert  forall boundary_zone z, level t in archive:
                level ∈ [global_frontier_min - K_max + 1, global_frontier_max]
        assert invariants_I1_to_I7(inner)   # inherited
```

**Минимум 10⁵ сгенерированных Pattern-2 sequences** в CI per scheduler/comm/zoning PR (lower bar than Pattern 1's 10⁶ — Pattern 2 fuzzer wraps the inner one and runs ~10× slower).

В Pattern 1 bit 4 всегда 0; fuzzer работает как в M5 без изменений.
...
### 4.6. HaloSnapshot archive (Pattern 2, M7+)
...
struct HaloSnapshot {
    ...
    ZoneId         source_zone_id;       // peer's local zone id within its inner DAG
    TimeLevel      time_level;
    Version        source_version;       // peer's state version at the time of pack
    uint32_t       atom_count;
    std::vector<uint8_t>  payload;       // unpacked positions/velocities/types,
                                          // SoA layout matching state/SPEC §3.2
    uint64_t       received_seq;          // monotonic per-coordinator on register
};
...
```
HaloPacket arrives via comm.drain_halo_arrived()
    → coordinator unpacks payload (CRC32 verify per comm/SPEC §4.4)
    → register into ring buffer keyed by (source_subdomain_id, source_zone_id, time_level)
    → emit HaloArrived event
        → re-evaluate boundary deps (§2.5)
    → snapshot served to local boundary zones via fetch_peer_snapshot()
    → eviction once peer time_level falls outside [global_frontier_min, global_frontier_max + K_max]
```

**Архивная структура** (per `(source_subdomain_id, source_zone_id)` pair):

| Поле | Тип | Назначение |
|---|---|---|
| `slots` | `circular_buffer<HaloSnapshot, K_max>` | ring buffer last K_max levels |
| `oldest_level` | `TimeLevel` | level в slot 0; eviction sentinel |
| `newest_level` | `TimeLevel` | level в slot (size-1); insertion sentinel |
| `use_count` | `uint32_t` | сколько раз local boundary zones fetched (per slot) |

**Invariants:**

| # | Инвариант | Когда нарушение — bug |
|---|---|---|
| HA-1 | `newest_level - oldest_level + 1 ≤ K_max` (ring capacity) | overflow regis: must evict before insert |
| HA-2 | Регистрация одного и того же `(peer_zone, level)` дважды → reject + diagnostic | peer subdomain bug или corrupt comm |
| HA-3 | `fetch_peer_snapshot(level)` для `level < oldest_level` → возвращает `nullopt` + telemetry counter `coord.snapshot_too_old_total++` | inner scheduler выехал за K_max — цеп логического deadlock'а; watchdog обязан сработать |
| HA-4 | `fetch_peer_snapshot(level)` для `level > newest_level` → `nullopt`, **не error** (peer ещё не дошёл до этого уровня) | normal stall, инициирует callback registration |
| HA-5 | Eviction (`pop_front`) разрешён только если `use_count == 0` для этого слота | prevents use-after-free на in-flight fetch |

**Память:** для одного boundary zone на subdomain'е A архив ≈ `K_max × atom_count × atom_record_size`. Atom record (SoA, FP64) = 8 floats × 8 bytes = 64 bytes; для K_max=8 и `atom_count=200` (типичная boundary zone в T7 800k-atom benchmark) — ~100 KB per peer per zone. Для 2D mesh с 4 peers и 50 boundary zones — ~20 MB на subdomain. Acceptable per мастер-спец §4a.8.

**Determinism в Reference profile:**

- `received_seq` назначается **deterministic** счётчиком coordinator'а, не системным временем;
- eviction order — strictly `oldest_level` ascending (FIFO по time_level, **не** по wall clock);
- `register_boundary_snapshot` collision (HA-2) → hard error в Reference, advisory log в Production / Fast.

**Ссылка на comm:** wire format = `HaloPacket` (см. comm/SPEC §4.2); коэффициент конверсии `HaloPacket → HaloSnapshot` идёт через `coord.unpack_halo(packet) → HaloSnapshot`. Coord владеет unpack-кодом (subdomain-aware), comm владеет только pack/transport. Это сохраняет ownership boundary §8.2 мастер-специи.
...
| `HaloArrived` | `comm/` (Pattern 2) | coordinator unpacks → registers `HaloSnapshot` в archive (§4.6); inner re-evaluates dep_mask bit 4 (§2.5) для затронутых boundary zones |
...
5. **OQ-M7-1 — HaloSnapshot eviction triggering** (§4.6, T7.6 implementation): должна ли eviction случаться eagerly после `use_count → 0` или lazy при следующем `register_boundary_snapshot` overflow? Eager экономит память, lazy экономит lock contention. Решается замером в T7.11 mixed-scaling benchmark.
6. **OQ-M7-2 — `T_stall_max` default value** (§2.4 OC-6): мастер-спец §4a.8 не фиксирует numerical value. Предложение: 5×`T_inner_step_typical` (отсчитывается на calibrate-time через PerfModel). Финализируется в T7.7 после первого Pattern 2 multi-rank smoke.
7. **OQ-M7-3 — `register_boundary_snapshot` race window** (§2.4 OC-2): между Phase B `commit_completed()` и `register_boundary_snapshot()` есть короткое окно, в котором соседний subdomain может запросить snapshot и получить `nullopt` спиаидерно. Для Reference profile это ОК (callback retry); для Production — нужен ли atomic publish? Решается в T7.6.
...
- **2026-04-19** — **T7.2 — Pattern 2 SPEC integration (M7 entry).**
  ...
  Pattern-2-shaped holes left by M5/M6 forward declarations:
  - **§2.4** authored — `OuterSdCoordinator` canonical interface
    transcribed from master spec §12.7a + 6-row behavioural contract
    (OC-1..OC-6) covering non-blocking semantics, idempotence, archive
    capacity bounds, monotonic global frontier, and the boundary-specific
    watchdog separation from §8 inner deadlock detection. Pattern 1
    contract preserved: `attach_outer_coordinator(nullptr)` is a legal
    initial state, the §5.1 boundary branch is skipped.
  - **§2.5** authored — `SubdomainBoundaryDependency` formalized as
    dep_mask **bit 4** (taxonomy-aligned with master §6.3 fifth dep kind);
    bits 0–3 retroactively assigned to spatial / temporal-frontier / cert
    / neighbor-freshness deps; release semantics on `HaloArrived` event;
    Pattern-2 fuzzer extension (≥10⁵ sequences, lower bar than Pattern 1
    10⁶ because the wrapper runs ~10× slower).
  - **§4.6** authored — `HaloSnapshot` type + ring-buffer archive (per
    `(peer_subdomain, peer_zone)`, capacity `K_max`) + 5-row invariant
    table (HA-1..HA-5) covering capacity, double-register rejection,
    too-old / too-new fetch behaviour, use-count-gated eviction. Memory
    bound: ~20 MB/subdomain on 800k-atom T7 benchmark. Determinism in
    Reference: `received_seq` deterministic counter, eviction
    strictly `oldest_level` ascending.
  - **§10.1 events table** — `HaloArrived` row expanded to describe the
    coordinator unpack → archive → dep_mask re-evaluation chain.
  - **§15 open questions** — added OQ-M7-1 (eviction trigger eager vs
    lazy), OQ-M7-2 (`T_stall_max` numeric default), OQ-M7-3
    (`register_boundary_snapshot` race window). All three resolve in
    T7.6 / T7.7 implementation tasks.
  - Cross-link: `comm/SPEC §4.2 + §6.4 + §14` (T7.2 sister edit) clarifies
    `HaloPacket` wire format ↔ `HaloSnapshot` archive role split per
    ownership boundary §8.2 master spec (coord owns unpack, comm owns
    transport).
