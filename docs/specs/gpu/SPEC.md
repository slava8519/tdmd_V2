...
**Module:** `gpu/`
**Status:** master module spec v1.0.21 (T8.6c shipped — warp-parallel kernel refactor of ui `add_uarraytot`, yi Phase-A over `idxz_max`, deidrj Phase-B warp-shuffle dedr reduction; T8.7 byte-exact gate preserved ≤ 1e-12 rel; MixedFast SNAP 316.0 → 208.6 ms/step on RTX 5080 W BCC 2000-atom, 1.51× cumulative; still 48.5× slower than LAMMPS KOKKOS `snap/kk` 4.30 ms/step; compute_uarray/duarray recurrences remain tid==0 pending v4. T8.9 MixedFastSnapOnly narrow-FP32 (D-M8-8 9.51e-6 force / 1.70e-9 PE / 3.94e-9 virial) unchanged. ctest green 21/21 SNAP across Fp64Reference+CUDA / MixedFast+CUDA / MixedFastSnapOnly+CUDA / CPU-strict)
...
| T8.6c-v4 | compute_uarray + compute_duarray recurrence parallelism (est. 2-3× more, byte-exact via commutative FP64 add) | Planned; pre-impl report pending |
