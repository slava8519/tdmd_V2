...
**Module:** `gpu/`
**Status:** master module spec v1.0.24 (T-opt-3b + T-opt-2 shipped 2026-04-21 — two post-T8.6c-v5 kernel-level optimizations landed as single commits after v5 Stage 3 closed the per-bond dispatch refactor. **T-opt-3b** (commits `e1a58e3` Stage A + `c380e10` Stage B) halves `snap_deidrj_bond_kernel` work via a paired-bond reverse index: the full bond list emits both (i→j) and (j→i) bonds, and under the pure-function compute_deidrj(Δr, weight, ylist_slab) semantics the dedr_peer of bond b equals dedr_own of its reverse bond b' bit-for-bit (Δr flips sign, rsq/r/rcut/z0/dz0dr invariant under i↔j, weights swap roles, ylist slab source becomes atom j=bond_i[b']). Kernel now computes own-side only per bond; `snap_force_gather_kernel` subtracts `d_dedr_own[reverse(b)]` in place of a separately computed `d_dedr_peer[b]`. Added `d_reverse_bond_index` to `SnapBondListGpu` (Pass 5 at build time), `test_bond_list_reverse_index_well_formed` validates the pairing invariant. **T-opt-2** (commit `63847c2`) parallelises `snap_yi_kernel` Phase B via jju-bucket CSR: host-side counting-sort of jjz values by their target jju, kernel tid-strides over idxu_max jju slots, each thread accumulates ybuf over its bucket range — no atomics because buckets are disjoint over jju; ascending-jjz order inside each bucket matches the legacy tid==0 sweep bit-for-bit. Cumulative RTX 5080 W BCC 2000-atom perf (median of 3, 100 steps): MixedFast 47.9 → **29.5 ms/step** (**1.62× over v5 Stage 3, 10.71× cumulative T8.6c**); Fp64Ref 59.6 → 37.0 ms/step (1.61×); MixedSnapOnly 48.0 → 29.7 ms/step (1.62×). Gap to LAMMPS KOKKOS `snap/kk` 4.30 ms/step narrows from 11.1× to **6.86×** (MixedFast) — another 38% of the remaining gap closed. T8.7 byte-exact gate (≤ 1e-12 rel) preserved by construction in both optimizations (paired-bond identity is pure-function equivalence; jju-bucket iteration is a stable-sort permutation of the legacy accumulation order). D-M8-8 MixedFast budget (≤ 1e-5 force / ≤ 1e-7 PE) preserved across both changes. ctest green 52/52 across Fp64Reference+CUDA / MixedFast+CUDA / MixedFastSnapOnly+CUDA)
...
| T8.6c | Warp-parallel kernel refactor — ui `add_uarraytot` / yi Phase-A over `idxz_max` / deidrj Phase-B dedr-reduction via warp-shuffle; T8.7 byte-exact preserved | **LANDED 2026-04-20 (v1/v2/v3)**: 1.51-1.56× cumulative on RTX 5080 W BCC 2000-atom; MixedFast 316→208.6 ms/step; compute_uarray/duarray still tid==0 pending v4 |
| T8.6c-v4 | compute_uarray + compute_duarray intra-layer (mb, k) parallelism; layer-sequential outer, 128-lane fill + symmetrize within each j-layer | **LANDED 2026-04-20**: 2.27× (MixedFast) / 2.67× (Fp64Ref) / 1.80× (MixedSnapOnly) single-commit; MixedFast 208.6 → 92.0 ms/step; cumulative T8.6c 3.43× / 4.09× / 2.81×; T8.7 byte-exact preserved. MixedFast beats LAMMPS CPU 1-rank (0.517×); vs LAMMPS KOKKOS GPU 4.30 ms/step still 21.4× gap |
| T8.6c-v5 | Per-bond dispatch refactor (Stages 1-3) matching LAMMPS KOKKOS `pair_snap_kokkos`'s launch shape: (1) device-resident `SnapBondListGpu` with CSR `atom_bond_start` and per-bond SoA (dx/dy/dz/rsq/type_i/type_j), emission order proven byte-identical to the CPU 3×3×3 stencil walk; (2) `snap_ui_bond_kernel <<<n_bonds, 128>>>` + `snap_ui_gather_kernel <<<n_atoms, 128>>>` (replaces `snap_ui_kernel`); (3) `snap_deidrj_bond_kernel <<<n_bonds, 128>>>` + `snap_force_gather_kernel <<<(n+127)/128, 128>>>` (replaces `snap_deidrj_kernel`). **Atomic-free** per gpu/SPEC §6.1 — exclusive per-bond storage + per-atom gather sums bond range in CSR order (= CPU order), preserving T8.7 ≤ 1e-12 byte-exact gate; MixedFast FP32 `sqrtf` narrowing mirrored into bond kernels to preserve D-M8-8 ≤ 1e-5/1e-7 rel | **LANDED 2026-04-21** (commits 4cf0202 / ac781ac / 6c5feb1 for Stages 1/2/3): RTX 5080 W BCC 2000-atom 100-step medians — MixedFast 92.0 → 47.9 ms/step (**1.92× T8.6c-v5 cumulative**, **6.60× cumulative T8.6c incl. v4**); Fp64Ref 109.6 → 59.6 ms/step (1.84×); MixedSnapOnly 116.0 → 48.0 ms/step (2.42×). Gap to LAMMPS KOKKOS `snap/kk` 4.30 ms/step narrows 21.4× → **11.1×** — 48% of the gap closed. Legacy path retained via `-DTDMD_SNAP_LEGACY_PERATOM=1`. ctest green 52/52 on all three flavors; new test `test_bond_list_matches_cpu_stencil_order` validates Stage 1 emission-order invariant |
| T-opt-3b | Halve `snap_deidrj_bond_kernel` work via paired-bond reverse index: the full bond list emits (i→j) as bond b AND (j→i) as bond b'. Under the pure-function semantics `compute_deidrj(Δr, weight, ylist_slab)`, dedr_peer[b] ≡ dedr_own[reverse(b)] bit-for-bit (Δr flips sign; rsq/r/rcut/z0/dz0dr invariant under i↔j; weights swap roles; ylist slab source becomes atom j=bond_i[b']). Kernel now computes own-side only per bond; `snap_force_gather_kernel` subtracts `d_dedr_own[reverse(b)]` in place of separately computed `d_dedr_peer[b]`. Stage A: `SnapBondListGpu` Pass 5 builds `d_reverse_bond_index`, new test `test_bond_list_reverse_index_well_formed` validates the pairing invariant. Stage B: kernel + gather trim | **LANDED 2026-04-21** (Stage A commit `e1a58e3`, Stage B commit `c380e10`): MixedFast 47.9 → 34.4 ms/step (**1.39×**, **8.0× gap**); byte-exact by pure-function equivalence (T8.7 ≤ 1e-12 rel preserved) |
| T-opt-2  | Parallelise `snap_yi_kernel` Phase B via jju-bucket CSR: host-side counting-sort of jjz values by their target jju (idxz_packed[jjz*10+9]) yields `idxz_jju_bucket_begin[idxu_max+1]` + `idxz_by_jju[idxz_max]`. Kernel Phase B: each thread tid-strides over idxu_max jju slots, accumulates `ybuf[jjz]` values over its bucket range into `ylist[jju]`. No atomics — buckets are disjoint over jju, so writes are exclusive. Ascending-jjz order inside each bucket matches legacy tid==0 sweep bit-for-bit | **LANDED 2026-04-21** (commit `63847c2`): MixedFast 34.4 → **29.5 ms/step** (**1.17×**, **6.86× gap**); cumulative post-v5 (T-opt-3b × T-opt-2) **1.62× over v5 Stage 3**, **10.71× cumulative T8.6c**. Byte-exactness preserved by stable-sort permutation invariant |
...
| T8.9  | MixedFast SNAP kernel — FP32 math + FP64 accum (Philosophy B) | **LANDED 2026-04-20**: D-M8-8 ≤ 1e-5 rel force / ≤ 1e-7 rel PE; narrowed to `sqrtf` only |
| T8.10 | T6 medium/large variants (1024/8192 atoms) — performance baseline vs LAMMPS CPU SNAP | **LANDED 2026-04-20**: real KOKKOS GPU baseline 4.30 ms/step; TDMD post-T8.6c 208.6 ms/step = 48.5× slower |
...
### 8.5. MixedFastSnapOnlyBuild — narrow-FP32 SNAP pair-math (T8.9 LANDED)

**Flavor policy per D-M8-4:**
- SNAP → `SnapGpuMixed` (narrow-FP32 pair-math)
- EAM → `EamAlloyGpu` (FP64 — tabulated Horner splines fail monotonicity in FP32 per D-M6-8)
- NL, VV, scheduler, comm — identical to `Fp64ReferenceBuild`

**T8.9 FP32 scope — the final landed version is narrower than the pre-impl report proposed.** Only one FP32 site survived D-M8-8 empirical measurement:

```cpp
// The only FP32 op in the entire SnapGpuMixed kernel path:
const float rsq_f = static_cast<float>(rsq);
const float r_f = sqrtf(rsq_f);
const double r = static_cast<double>(r_f);  // cast back; everything downstream is FP64
```

Everything else (`theta0`, `z0`, `dz0dr`, `sfac`, `dsfac`, sincos, all recurrences, all accumulators) is bit-for-bit identical to `SnapGpu` FP64.

**Scope evolution — why `__sincosf` + FP32 z0 was rejected:**
- Wider-FP32 exploratory trial (also `__sincosf` + `z0_f` + `dz0dr_f` + FP32 sfac) measured force worst rel = 2.1e-5 on T6 tungsten — **above the D-M8-8 1e-5 cap**.
- Root cause: `__sincosf`'s ~1 ULP rel error (~6e-8) feeds `z0 = r · cos(θ)/sin(θ)`, and `z0` enters the VMK U-recurrence multiplicatively. A length-O(165) bispectrum dot product at twojmax=8 compounds the per-pair error to ~2e-5 on force.
- Narrowing to `sqrtf` alone keeps `r`'s rel error at ~6e-8 but all downstream scalars are FP64, bounding the compounding. Measured: force 9.51e-6, PE 1.70e-9, virial 3.94e-9.

**Throughput honesty:**
SNAP pair-math is <5 % of total SNAP runtime (U-recurrence + bispectrum dot products dominate). Narrow-FP32 `sqrtf` therefore yields a single-digit-% throughput win at best. `MixedFastSnapOnlyBuild` remains opt-in precisely because the speedup is modest. Real SNAP throughput gains come from `Fp64ProductionBuild` + dense-cutoff GPU path (T8.6b landed). Phase B (full-FP32 bispectrum) is deferred to a §D.17 procedure contingent on T8.11 cloud-burst perf pressure.

**Acceptance gate (D-M8-8 dense-cutoff):**
- Force per-atom L∞ rel ≤ 1.0e-5
- Total PE rel ≤ 1.0e-7
- Virial Voigt rel-to-max-component ≤ 5.0e-6
- Exercised by `tests/gpu/test_snap_mixed_fast_within_threshold.cpp` on T6 fixture (2000-atom rattled BCC-W, `W_2940_2017_2.snap*`). Test bypasses the flavor-dispatched adapter (constructs `SnapGpu` + `SnapGpuMixed` directly) so D-M8-8 is measured on every flavor's CI pass.

**Regression test handling on this flavor:**
- `test_snap_gpu_bit_exact` (D-M8-13 1e-12 rel CPU↔GPU) — `#ifdef TDMD_FLAVOR_MIXED_FAST_SNAP_ONLY` guard → `SUCCEED` + early return. Adapter dispatches to SnapGpuMixed, so CPU FP64 ≡ GPU at 1e-12 is structurally impossible on this flavor; D-M8-8 supersedes.
- `test_snap_gpu_functional` (1e-10 rel CPU↔GPU diff) — same guard pattern.
- `test_eam_mixed_fast_within_threshold` — unchanged (EAM stays FP64 on this flavor).
...
| 2026-04-20 | v1.0.20 | **T8.9 landed — `SnapGpuMixed` narrow-FP32 SNAP pair-math kernel for `MixedFastSnapOnlyBuild`.** §8.5 new — full MixedFast SNAP semantics. Final FP32 scope is a single op (`r = sqrtf(static_cast<float>(rsq))` in `snap_ui_kernel` + `snap_deidrj_kernel`) — narrower than the pre-impl report because exploratory wider-FP32 (also `__sincosf` + `z0_f` + `dz0dr_f` + `compute_sfac_device_f32`) measured force worst rel 2.1e-5 on T6 tungsten, above the D-M8-8 1e-5 cap. Root cause: `__sincosf`'s ~1 ULP error (~6e-8 rel) feeds `z0 = r·cos(θ)/sin(θ)`, and `z0` enters the VMK U-recurrence multiplicatively; a length-O(165) bispectrum dot product compounds the per-pair error to ~2e-5 on force. Narrowing to `sqrtf` only keeps per-pair rel error at ~6e-8 with all downstream FP64, bounding the compounding. Measured residuals on T6 2000-atom rattled BCC-W: **force 9.51e-6 (tol 1e-5), PE 1.70e-9 (tol 1e-7), virial rel-to-max 3.94e-9 (tol 5e-6)**. `src/gpu/snap_gpu_mixed.cu` (~1400 lines, cloned from `snap_gpu.cu` with one FP32 edit per kernel) + `src/gpu/include/tdmd/gpu/snap_gpu_mixed.hpp` (PIMPL header, byte-identical public interface to `SnapGpu`). `SnapGpuAdapter` typedef dispatch: `using SnapGpuActive = SnapGpuMixed;` under `TDMD_FLAVOR_MIXED_FAST_SNAP_ONLY`, else `SnapGpu`. `EamAlloyGpuAdapter` explicit SnapOnly-guard — EAM stays FP64 on SnapOnly flavor (tabulated Horner splines fail monotonicity in FP32 per D-M6-8). `tests/gpu/test_snap_mixed_fast_within_threshold.cpp` — three D-M8-8 gates (force L∞ / PE / virial), bypasses flavor-dispatched adapter to measure D-M8-8 on every flavor's CI pass. `test_snap_gpu_bit_exact` + `test_snap_gpu_functional` now `SUCCEED`-skip under `TDMD_FLAVOR_MIXED_FAST_SNAP_ONLY` (D-M8-8 supersedes D-M8-13 for SNAP on this flavor). `cmake/BuildFlavors.cmake` message rewritten: "T8.9 kernel split pending" → "narrow-FP32 pair-math (T8.9)". **Realized throughput lever is modest** — SNAP pair-math is <5 % of SNAP runtime, the U-recurrence + bispectrum dot products dominate. Real SNAP throughput gains come from `Fp64ProductionBuild` + dense-cutoff GPU path (T8.6b landed). MixedFastSnapOnlyBuild remains opt-in; Phase B (full-FP32 bispectrum) deferred to §D.17 procedure conditional on T8.11 cloud-burst perf pressure. All 51 ctest gates green on Fp64ReferenceBuild, MixedFastBuild, MixedFastSnapOnlyBuild (+1 structural skip `test_t4_nve_drift` — MixedFast-only by design). |
...
- **T8.9** → §8.5 new — `SnapGpuMixed` narrow-FP32 pair-math kernel for `MixedFastSnapOnlyBuild`. Scope narrowed to a single FP32 site (`r = sqrtf(rsq)`) after empirical measurement — wider FP32 (sincosf + z0 + dz0dr + sfac) failed D-M8-8 at 2.1e-5 force rel via bispectrum amplification. Measured on T6 2000-atom BCC W: force 9.51e-6 (budget 1e-5) / PE 1.70e-9 (budget 1e-7) / virial 3.94e-9 (budget 5e-6). `test_snap_gpu_bit_exact` + `test_snap_gpu_functional` gated with `#ifdef TDMD_FLAVOR_MIXED_FAST_SNAP_ONLY` SUCCEED-skip (D-M8-8 supersedes D-M8-13 on this flavor). CMake status message rewritten. Realized throughput lever is modest — pair-math is <5% of SNAP runtime; MixedFastSnapOnlyBuild remains opt-in (**done — v1.0.20**).
- **T8.6c** → §7.5 warp-parallel kernel refactor shipped in three commits (2026-04-20): **v1** (`1c22694`) `snap_ui_kernel` `add_uarraytot` block-parallel (5.4 % kernel share — measured flat end-to-end, structurally capped); **v2** (`ab0bcff`) `snap_yi_kernel` two-phase split — Phase A parallel-over-`idxz_max` compute of `ztmp`/`zlist`/`βj·ztmp` into new `ybuf_r`/`ybuf_i` shared buffers; Phase B tid==0 sequential `ylist[jju] += ybuf[jjz]` in original jjz order (28-31 % wall-time cut); **v3** (`88d23fb`) `snap_deidrj_kernel` Phase B: `dedr = Σ(dulist·ylist)` triple-dot via warp-shuffle tree reduction (128-thread strided walk of (j,mb,ma) triples + per-warp shared fan-in `warp_sums[4][3]` + tid==0 cross-warp sum × 2.0), OWN and PEER sides both parallelized; Phase A compute_uarray/compute_duarray remain tid==0 and are the new bottleneck (v4 scope). Shmem allocation raised from 48 KB to 160 KB with `cudaFuncSetAttribute(MaxDynamicSharedMemorySize)` opt-in (twojmax=8 → `idxz_max=2386` exceeds default limit). Deleted unused `compute_deidrj_device` helper. Byte-exact T8.7 gate preserved ≤ 1e-12 rel across all 21/21 SNAP tests in all 3 flavors. **Measured speedup (scout_rtx5080, 100-step W SNAP, 2000 atoms)**: Fp64Reference 448.3 → 292.5 ms/step (1.53×); MixedFast 316.0 → 208.6 ms/step (1.51×); MixedFastSnapOnly 326.1 → 209.2 ms/step (1.56×). vs LAMMPS KOKKOS GPU 4.30 ms/step: TDMD MixedFast now **48.5× slower** (was 73.5× pre-T8.6c). M8 ≥ 20 % gate (≤ 3.44 ms/step) requires ~61× further speedup — single-GPU kernel tuning alone cannot close; T8.6c-v4 (compute_uarray/duarray recurrence parallel) estimated 2-3× more; remainder must come from multi-rank TD (**done — v1.0.21**).
- **T8.6c-v4** → §7.5 compute_uarray + compute_duarray intra-layer parallelism (2026-04-20). Added two new device helpers in `src/gpu/snap_gpu_device.cuh`: `compute_uarray_parallel_device(tid, stride, ...)` and `compute_duarray_parallel_device(...)`. Each helper keeps the outer j-layer loop sequential (layer j reads layer j-1) and parallelises the intra-layer `(mb, k)` fill + `(mb, ma)` symmetrize passes over the 128 block threads with `__syncthreads()` between layers. Byte-exactness is preserved by construction: per-position writes in the parallel fill set `ulist_r[P] = real; ulist_i[P] = imag` where `real = v_b + v_a` (both `v_b` from k≥1 branch and `v_a` from k<j branch accumulated in per-thread locals), semantically identical to CPU's `ulist[P] = v_b; ulist[P] += v_a` two-step write because FP64 add is deterministic under fixed inputs. `compute_duarray_parallel_device` extends the same pattern with the vector k-loop (3D derivative) inside each position body + a parallelised sfac/dsfac scaling tail. Call sites updated in three kernels across two files: `snap_gpu.cu` snap_ui_kernel (L397–L430), `snap_gpu.cu` snap_deidrj_kernel OWN (L794–L849) + PEER (L879–L913); same mirror in `snap_gpu_mixed.cu`. Added `__shared__` scalar stashing (`r_sh`, `z0_sh`, `dz0dr_sh`, `rcut_sh`, `wj_sh`, `ui_r_sh`, etc.) so tid==0's scalar setup is visible to all lanes during the parallel recurrence. T8.7 byte-exact gate preserved ≤ 1e-12 rel; D-M8-8 threshold preserved ≤ 1e-5 rel force. **Measured speedup (scout_rtx5080, 1000-step median-of-3 W SNAP, 2000 atoms)**: Fp64Reference 292.5 → 109.6 ms/step (**2.67×**); MixedFast 208.6 → 92.0 ms/step (**2.27×**); MixedFastSnapOnly 209.2 → 116.0 ms/step (1.80×). **Cumulative T8.6c (v1+v2+v3+v4)**: Fp64Ref 4.09×, MixedFast 3.43×, MixedSnapOnly 2.81×. Gap to LAMMPS KOKKOS `snap/kk` 4.30 ms/step: MixedFast now 21.4× slower (was 48.5×); MixedFast first flavor that **beats LAMMPS CPU 1-rank** (0.517× = 1.94× faster vs 1.77× slower pre-T8.6c). MixedSnapOnly regression (1.80× < 2.27× of MixedFast) reveals the T8.9 narrow-FP32 setup path overhead — documented in RESULTS.md "T8.6c-v4 regression anomaly" section; T8.6c-v5 restructures the setup path and subsumes. M8 gate still needs ~21× further speedup on single RTX 5080 — T8.6c-v5 per-bond dispatch refactor (deferred) is the remaining single-GPU lever; closing the gap without v5 requires multi-rank TD or cloud burst (**done — v1.0.22**).
- **T8.6c-v5** → §7.5 per-bond dispatch refactor, Stages 1-3 (2026-04-21). Stage 1 (`4cf0202`): device-resident `SnapBondListGpu` (`src/gpu/snap_bond_list_gpu.{hpp,cu}`, `tdmd/gpu/snap_bond_list_gpu.hpp`) with CSR `d_atom_bond_start[n+1]` + per-bond SoA (`d_bond_{i,j,type_i,type_j,dx,dy,dz,rsq}`); 4-pass build-from-device (cell-atom walk → prefix sum → populate → validate) with emission order proven byte-identical to the legacy cell-stencil walk; new test `test_bond_list_matches_cpu_stencil_order` (CPU stencil oracle → download + compare). Stage 2 (`ac781ac`): `snap_ui_bond_kernel <<<n_bonds, 128>>>` writes exclusive `d_ulist_bond[b, k] = sfacwj * ulist[k]` per bond; `snap_ui_gather_kernel <<<n_atoms, 128>>>` sums each atom's bond range in CSR order into `d_ulisttot` (replaces legacy `snap_ui_kernel`). Stage 3 (`6c5feb1`): `snap_deidrj_bond_kernel <<<n_bonds, 128>>>` computes both dedr_own and dedr_peer per bond (128-lane triple-dot over dulist·ylist via warp-shuffle tree), writes exclusive `d_dedr_{own,peer}[b*3+k]`; `snap_force_gather_kernel <<<(n+127)/128, 128>>>` (1 thread/atom) walks atom bond range summing `fx += d_dedr_own - d_dedr_peer` (replaces legacy `snap_deidrj_kernel`). **Atomic-free** per gpu/SPEC §6.1 — exclusive per-bond storage + per-atom gather sums bond range in CSR order = CPU cell-stencil walk order, preserving the T8.7 ≤ 1e-12 rel byte-exact gate. MixedFast FP32 `sqrtf` narrowing mirrored 1:1 into bond kernels (scalar init only) to preserve D-M8-8 ≤ 1e-5/1e-7 rel. Legacy per-atom kernels retained behind `-DTDMD_SNAP_LEGACY_PERATOM=1` for A/B validation. **Measured speedup (scout_rtx5080, 100-step median-of-3 W SNAP, 2000 atoms)**: Fp64Reference 109.6 → 59.6 ms/step (**1.84×**); MixedFast 92.0 → 47.9 ms/step (**1.92×**); MixedFastSnapOnly 116.0 → 48.0 ms/step (**2.42×**). **Cumulative T8.6c (v1+v2+v3+v4+v5)**: Fp64Ref 7.52×, MixedFast 6.60×, MixedSnapOnly 6.79×. Gap to LAMMPS KOKKOS `snap/kk` 4.30 ms/step: 21.4× → **11.1×** slower — 48% of the remaining gap closed in a single milestone. ctest green 52/52 on all three flavors (**done — v1.0.23**).
- **T-opt-3b + T-opt-2** → §7.5 two post-v5 kernel-level optimizations (2026-04-21). **T-opt-3b** halves `snap_deidrj_bond_kernel` work via paired-bond reverse index: the full bond list emits both (i→j) and (j→i) bonds; under the pure-function compute_deidrj(Δr, weight, ylist_slab) semantics, dedr_peer[b] ≡ dedr_own[reverse(b)] bit-for-bit (Δr flips sign, rsq/r/rcut/z0/dz0dr invariant under i↔j, weights swap roles, ylist slab source becomes atom j=bond_i[b']). Kernel now computes own-side only per bond; `snap_force_gather_kernel` subtracts `d_dedr_own[reverse(b)]` in place of a separately computed `d_dedr_peer[b]`. Stage A (`e1a58e3`): `SnapBondListGpu` Pass 5 builds `d_reverse_bond_index` (kernel-built pairing index; emit-order invariance of full list ensures each bond has a unique reverse); new test `test_bond_list_reverse_index_well_formed` validates the pairing invariant. Stage B (`c380e10`): kernel signature drops `d_dedr_peer` param, removes ~60 lines of PEER-side code; gather reads `d_dedr_own[reverse(b)*3+k]` instead. **T-opt-2** (`63847c2`) parallelises `snap_yi_kernel` Phase B via jju-bucket CSR: host-side counting-sort of jjz values by their target jju (= `idxz_packed[jjz*10+9]`) yields `FlatIndexTables::idxz_jju_bucket_begin[idxu_max+1]` + `idxz_by_jju[idxz_max]`. Kernel Phase B: each thread tid-strides over idxu_max jju slots, accumulates ybuf over its bucket range into ylist[jju]. No atomics — buckets are disjoint over jju, so writes are exclusive; ascending-jjz order inside each bucket matches the legacy tid==0 sweep bit-for-bit. **Measured speedup (scout_rtx5080, 100-step median-of-3 W SNAP, 2000 atoms)**: Fp64Reference 59.6 → 37.0 ms/step (**1.61×**); MixedFast 47.9 → **29.5 ms/step** (**1.62×**); MixedFastSnapOnly 48.0 → 29.7 ms/step (**1.62×**). **Cumulative post-v5**: 1.62× (≈ T-opt-3b 1.39× × T-opt-2 1.17×). **Cumulative T8.6c+T-opt (v1..v5 + T-opt-3b + T-opt-2)**: Fp64Ref 12.11×, MixedFast 10.71×, MixedSnapOnly 10.98×. Gap to LAMMPS KOKKOS `snap/kk` 4.30 ms/step: 11.1× → **6.86×** slower (MixedFast) — another 38% of the remaining gap closed. MixedFast now **2.81× faster than LAMMPS CPU 1-rank** (8.3 ms/step). T8.7 byte-exact gate (≤ 1e-12 rel) preserved by construction in both optimizations (pure-function equivalence for T-opt-3b; stable-sort permutation for T-opt-2). D-M8-8 MixedFast budget preserved. ctest green 52/52 on all three flavors. M8 ≥20% gate (≤ 3.44 ms/step) still requires ~8.6× further single-GPU speedup — remaining levers are (a) multi-rank TD (the structural M7/M8 angle) and (b) algorithmic wins from TD's multi-step scheduling (zones on coarser dt cost proportionally less per wall-second) (**done — v1.0.24**).
