...
**Module:** `gpu/`
**Status:** master module spec v1.0.22 (T8.6c-v4 shipped — compute_uarray/compute_duarray intra-layer (mb, k) parallelism; layer-sequential outer recurrence with 128-lane fill + symmetrize inside each j-layer; byte-exact by construction; T8.7 byte-exact gate preserved ≤ 1e-12 rel; MixedFast SNAP 208.6 → 92.0 ms/step on RTX 5080 W BCC 2000-atom, **2.27× single-commit / 3.43× cumulative T8.6c**; Fp64Ref 292.5 → 109.6 ms/step (2.67×); MixedSnapOnly 209.2 → 116.0 ms/step (1.80×, reveals T8.9 narrow-FP32 overhead now that recurrence is no longer the bottleneck — documented regression in t6 RESULTS.md). Gap to LAMMPS KOKKOS `snap/kk` 4.30 ms/step narrows from 48.5× to **21.4×** (MixedFast); MixedFast first flavor that beats LAMMPS CPU 1-rank (1.94× faster vs 1.77× slower pre-T8.6c). T8.6c-v5 per-bond-dispatch refactor (AoSoA neighbour layout, per-bond team parallelism à la LAMMPS KOKKOS) deferred — requires bond-list pre-pass + atomicAdd force reduction + kernel-launch shape rewrite. T8.9 MixedFastSnapOnly narrow-FP32 (D-M8-8 9.51e-6 force / 1.70e-9 PE / 3.94e-9 virial) unchanged. ctest green 21/21 SNAP across Fp64Reference+CUDA / MixedFast+CUDA / MixedFastSnapOnly+CUDA / CPU-strict)
...
| T8.6c-v5 | Per-bond team dispatch (à la LAMMPS KOKKOS pair_snap_kokkos): AoSoA neighbour pre-pass + per-bond ComputeUi/FusedDuidrjDeidrj kernels + atomicAdd force reduction; est. ~10-15× further | **DEFERRED**: architectural rewrite, requires new bond-list + kernel-launch shape + atomicAdd(double) force reduction (currently forbidden for determinism — needs D-M6-17 exception or scratch-based reduce); ≥ 1 day focused work, byte-exact re-validation required at each stage |
