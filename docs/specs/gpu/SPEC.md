...
**Module:** `gpu/`
**Status:** master module spec v1.0.23 (T8.6c-v5 Stages 1-3 shipped 2026-04-21 — per-bond dispatch refactor completed as atomic-free reduce-then-scatter (no `atomicAdd(double*, double)` — gpu/SPEC §6.1 preserved): Stage 1 built a device-resident SnapBondListGpu (CSR `atom_bond_start` + bond SoA dx/dy/dz/rsq/type_i/type_j), with the emission order proven byte-identical to the legacy cell-stencil walk (`test_bond_list_matches_cpu_stencil_order`); Stage 2 replaced `snap_ui_kernel` with `snap_ui_bond_kernel <<<n_bonds, 128>>>` writing exclusive per-bond `d_ulist_bond[b, k] = sfacwj * ulist[k]` + `snap_ui_gather_kernel <<<n_atoms, 128>>>` summing each atom's bond range (CSR order = CPU order, byte-exact); Stage 3 replaced `snap_deidrj_kernel` with `snap_deidrj_bond_kernel <<<n_bonds, 128>>>` (computes both dedr_own and dedr_peer per bond, writes exclusive `d_dedr_{own,peer}[b*3+k]`) + `snap_force_gather_kernel <<<(n+127)/128, 128>>>` (1 thread/atom, walks bond range summing fx += own - peer and virial from own × bond_dΔr). Cumulative RTX 5080 W BCC 2000-atom perf (median of 3, 100 steps): MixedFast 92.0 → 47.9 ms/step (**1.92× T8.6c-v5**, **6.60× cumulative T8.6c**); Fp64Ref 109.6 → 59.6 ms/step (1.84×); MixedSnapOnly 116.0 → 48.0 ms/step (2.42×). Gap to LAMMPS KOKKOS `snap/kk` 4.30 ms/step narrows from 21.4× to **11.1×** (MixedFast) — 48% of the gap closed in a single milestone. T8.7 byte-exact gate (≤ 1e-12 rel) preserved by the CSR-order = CPU-order invariant; D-M8-8 MixedFast narrow-FP32 budget (≤ 1e-5 force / ≤ 1e-7 PE) preserved (FP32 `sqrtf` mirrored 1:1 into `snap_ui_bond_kernel` and `snap_deidrj_bond_kernel`). Legacy per-atom kernels retained behind `-DTDMD_SNAP_LEGACY_PERATOM=1` for A/B validation. ctest green 52/52 across Fp64Reference+CUDA / MixedFast+CUDA / MixedFastSnapOnly+CUDA)
